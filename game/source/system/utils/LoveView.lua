...
local function updateView()
    ...
    for _, view in ipairs(LoveView.stack) do
        ...
        if fileinfo and (fileinfo.modtime > view.modtime or view.forceReload) then
            local ok, loadedChunk = pcall(love.filesystem.load, view.path)

            if view.forceReload then
                view.forceReload = false
            end
...
function LoveView.getViewByIndex(idx)
    return LoveView.stack[idx]
end

function LoveView.reloadViewByIndex(idx)
    LoveView.stack[idx].forceReload = true
end

function LoveView.reloadAll()
    for idx, view in ipairs(LoveView.stack) do
        view.forceReload = true
    end
end
